...
<style>
    ...
    <div class="flex justify-between items-center my-2">
        <div class="">
            <form action="{{ route('import.donations') }}" method="POST" enctype="multipart/form-data">
                @csrf
        ...
        <div class="mt-6">
            <a href="{{route('export.donations')}}" class="bg-green-700 hover:bg-green-800 text-white font-bold py-2 px-4 rounded-lg">
                <i class="fas fa-download mr-2"></i> Exportar excel
            </a>
        </div>

        <form action="{{ route('donations.table') }}" method="GET" class="flex items-center mt-5">
    ...
    <div class="container mx-auto my-10">
        <h1 class="text-3xl font-semibold mb-6">Tabla Donaciones</h1>
                ...
                <tbody>
                    @foreach ($donations as $donation)
                        ...
                        <tr class="transition-all hover:bg-gray-100">
                            <td class="py-3 px-4">{{ $donation->code }}</td>
                            <td class="py-3 px-4">{{ $donation->title }}</td>
                            <td class="py-3 px-4">{{ $donation->author }}</td>
                            <td class="py-3 px-4">{{ $donation->editorial }}</td>
                            <td class="py-3 px-4">{{ $donation->amount }}</td>
                            <td class="py-3 px-4">{{ $donation->category }}</td>
                            <td class="py-3 px-4">{{ $donation->area }}</td>
                            <td class="py-3 px-4">
                                @if ($donation->status == 'well')
                                    <span
                                        class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100">
                                        Bueno
                                    </span>
                                @elseif ($donation->status == 'regular')
                                    <span
                                        class="px-2 py-1 font-semibold leading-tight text-green-700 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-100">
                                        Regular
                                    </span>
                                @elseif ($donation->status == 'bad')
                                    <span
                            ...
                            <td class="py-3 px-4">{{ $donation->clasifpgc }}</td>
                            <td class="py-3 px-4">
                                @if ($donation->activite == 'reference_material')
                                    <span>Material de Referencia</span>
                                @elseif ($donation->activite == 'investigation')
                                    <span>Invenstigacion</span>
                                @elseif ($donation->activite == 'teaching')
                                    <span>Enseñanza</span>
                                @elseif ($donation->activite == 'consultation')
                                    <span>Consultar</span>
                                @elseif ($donation->activite == 'languagues')
                                    <span>Lenguas</span>
                                @elseif ($donation->activite == 'languagues')
                                    <span>Lectura</span>
                                @else
                            ...
                            <td class="py-3 px-4">{{ $donation->year }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
