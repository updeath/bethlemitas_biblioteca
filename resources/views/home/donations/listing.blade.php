    ...
    <div class="flex flex-wrap justify-start">
        @foreach ($donations as $donation)
            <div class="max-w-xs rounded overflow-hidden shadow-lg m-2 ">
                @if ($donation->image)
                    <img class="w-full max-w-32 h-auto mx-auto" src="{{ asset($donation->image) }}"
                        alt="{{ $donation->title }}">
                @else
                    <img class="w-full max-w-32 h-auto mx-auto" src="/img/logo.png" alt="{{ $donation->title }}">
                @endif

                <div class="px-4 py-2">
                    <div class="font-bold text-lg mb-1">Información del libro</div>
                    <p class="text-gray-700 text-sm">
                        <b>Codigo: </b>{{ $donation->code }}
                    </p>
                    <p class="text-gray-700 text-sm">
                        <b>Titulo: </b>{{ $donation->title }}
                    </p>
                    <p class="text-gray-700 text-sm">
                        <b>Autor: </b>{{ $donation->author }}
                    </p>
                    <p class="text-gray-700 text-sm">
                        <b>Editorial: </b>{{ $donation->editorial }}
                    </p>
                </div>
                <div class="flex justify-between items-center px-4 pt-2 pb-1">
                    <button
                        ...
                        onclick="showModal('{{ $donation->title }}',
                                    '{{ $donation->author }}',
                                    '{{ $donation->editorial }}',
                                    '{{ asset($donation->image) }}',
                                    '{{ $donation->code }}',
                                    '{{ $donation->amount }}',
                                    '{{ $donation->category }}',
                                    '{{ $donation->area }}',
                    ...
                    <!-- ... Tu código existente ... -->
                    <a href="{{ route('donations.edit', ['donation' => $donation->id]) }}"
                        class="inline-block bg-blue-500 hover:bg-blue-600 rounded-full px-2 py-1 text-xs font-semibold text-white mr-1 mb-1">Editar
                        </a>
                        <form id="discard-form-{{ $donation->id }}" action="{{ route('discard.book', ['id' => $donation->id]) }}" method="post">
                            @csrf
                            <button type="button" class="inline-block bg-red-500 hover:bg-red-600 rounded-full px-2 py-1 mt-3 text-xs font-semibold text-white  btn-show-confirm" data-id="{{ $donation->id }}">Descartar</button>
                        </form>
                        <form id="send-to-inventory-form-{{ $donation->id }}"
                            action="{{ route('donations.send_to_inventory', ['id' => $donation->id]) }}" method="post">
                            @csrf
                            <button type="submit"
                                ...
                                data-id="{{ $donation->id }}">Enviar a inventario</button>
                        </form>
...
<script>
    function showModal(title, author, editorial, image, code, amount, category, area, status, clasifpgc, activite, year) {
    ...
    <script>
        $(document).ready(function () {
            $('.btn-show-confirm').click(function (e) {
                ...
                Swal.fire({
                    title: '¿Estás seguro de descartar este libro?',
                    text: '¡No podrás revertir esto!',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#d33',
                    cancelButtonColor: '#6c757d',
                    confirmButtonText: 'Sí, descartarlo',
                    cancelButtonText: 'Cancelar'
                }).then((result) => {
                    if (result.isConfirmed) {
                        $('#discard-form-' + id).submit();
                    }
                });
            });
        });
...
<script>
    $(document).ready(function () {
        $('.btn-show-confirm').click(function (e) {
            ...
            Swal.fire({
                title: '¿Estás seguro de enviar este libro al inventario?',
                text: '¡No podrás revertir esto!',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#198754',
                cancelButtonColor: '#6c757d',
                confirmButtonText: 'Sí, enviar al inventario',
                cancelButtonText: 'Cancelar'
            }).then((result) => {
                if (result.isConfirmed) {
                    $('#send-to-inventory-form-' + id).submit();
                }
            });
        });
    });
